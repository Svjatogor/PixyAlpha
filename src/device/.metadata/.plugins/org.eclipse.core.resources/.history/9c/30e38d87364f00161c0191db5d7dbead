...
int32_t cam_getFrameChirpFlags(const uint8_t &type, const uint16_t &xOffset, const uint16_t &yOffset, const uint16_t &xWidth, const uint16_t &yWidth, Chirp *chirp, uint8_t renderFlags)
{
	...
	// debug information
	// print raw frame second and penultimate
...
void inverceImage(const uint16_t &xWidth, const uint16_t yWidth, int32_t len, uint8_t* frame){
	for (uint16_t i = 0; i < yWidth; i++){
		for (uint16_t j = 0; j < xWidth; j++){
			frame[len + i * xWidth + j] = 255 - frame[len + i * xWidth + j];
		}
	}
}
...
//			y[(i - i/2) * 80 + (j - j/2)] = calcY(r, g, b);
//			cprintf("Y[%d][%d] = %d", i - i/2, j - j/2, y[(i - i/2)*80 + j - j/2]);
//		}
//	}
